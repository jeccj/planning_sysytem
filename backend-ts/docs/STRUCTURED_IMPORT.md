...
### 3.2 字段规则
- `name`：必填
- `type`：必填，支持：`Classroom` / `Hall` / `Lab`（也支持中文：教室/礼堂/实验室）
- `capacity`：必填，正整数
- `building_name`：必填
- `floor_label`：必填
- `room_code`：必填
...
- `admin_username`：必填，且必须在 users 中存在

### 3.3 场馆 upsert 业务键
- `type + building_name + floor_label + room_code`
...
- `users.csv: unexpected headers -> extra_col`
- `users.csv: file is empty`
- `users.csv line 8: floor_admin requires managed_building or managed_floor`
