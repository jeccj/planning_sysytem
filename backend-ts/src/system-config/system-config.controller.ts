...
import {
  getDefaultLlmPrompts,
  LLM_PROMPT_CONFIG_KEYS,
  LLM_PROMPT_LOCKED_KEYS,
...
} = require('../../scripts/export-structured');
...
export class SystemConfigController {
  ...
  async updateConfigs(
    ...
    for (const config of body.configs) {
      ...
      const editableDefaults = defaultPrompts.editable_defaults as Record<
        string,
        string
      >;
      const currentValue =
        currentValueInDb === null
  ...
  async importStructured(
    ...
    body: {
      dry_run?: string;
      replace_classrooms?: string;
      prune_missing_users?: string;
      prune_missing_venues?: string;
    },
  ) {
    ...
    try {
      const result = await this.runStructuredImportWithMaintenance(
        payload,
        user,
      );
      return {
  ...
  async startStructuredImport(
    ...
    void this.runStructuredImportWithMaintenance(
      payload,
      user,
      (event: any) => {
  ...
  async exportStructuredUsers(@Res({ passthrough: true }) res: Response) {
    ...
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="${this.buildExportFilename('users')}"`,
      );
  ...
  async exportStructuredVenues(@Res({ passthrough: true }) res: Response) {
    ...
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="${this.buildExportFilename('venues')}"`,
      );
      res.setHeader('X-Structured-Export-Rows', String(result.rows || 0));
      return result.csvText;
    } catch (error) {
  ...
  private updateImportJob(
    jobId: string,
    patch: Partial<StructuredImportJobState>,
  ) {
  ...
  private parseImportPayload(
    ...
    if (!usersFile && !venuesFile) {
      throw new BadRequestException(
        '请上传 users_file 或 venues_file 至少一个 CSV 文件',
      );
    }
    ...
    return {
      ...
      pruneMissingVenues: this.parseBooleanFlag(
        body?.prune_missing_venues,
        true,
      ),
    };
  }
  ...
  private readUploadedFileText(file: any): string {
    ...
    throw new BadRequestException(
      `无法读取上传文件: ${file?.originalname || 'unknown'}`,
    );
  }
}
