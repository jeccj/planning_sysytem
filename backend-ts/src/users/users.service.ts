...
export class UsersService {
  ...
  sanitizeIdentityLast6(raw?: string): string {
    ...
    if (cleaned.length !== 6) {
      throw new BadRequestException(
        'identity_last6 must be exactly 6 characters (digits or X)',
      );
    }
    return cleaned;
  }
  ...
  async create(createUserDto: CreateUserDto): Promise<User> {
    if (createUserDto.role === UserRole.FLOOR_ADMIN) {
      throw new BadRequestException(
        'floor_admin role is deprecated, please use venue_admin',
      );
    }
    ...
    const user = this.userRepository.create({
      ...
      managedBuilding: isScopedAdmin
        ? createUserDto.managed_building || ''
        : '',
  ...
  async update(id: number, updateUserDto: UpdateUserDto): Promise<User> {
    ...
    if (Object.prototype.hasOwnProperty.call(updateUserDto, 'identity_last6')) {
      user.identityLast6 = this.sanitizeIdentityLast6(
        updateUserDto.identity_last6,
      );
    }
