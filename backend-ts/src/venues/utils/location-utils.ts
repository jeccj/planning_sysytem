...
export function parseVenueLocation(
  ...
  const buildingName = buildingMatch?.[1]
    ...
    : DEFAULT_BUILDING;
  const floorMatch = raw.match(
    /((?:B\d+|\d+)[F层]|(?:地上|地下)?\d+层|[一二三四五六七八九十]+层)/,
  );
...
export function buildVenueLocation(
  buildingName?: string,
  floorLabel?: string,
  roomCode?: string,
  fallback?: string,
  ...
  const pieces = [
    normalizeText(buildingName),
    normalizeText(floorLabel),
    normalizeText(roomCode),
  ].filter(Boolean);
  if (pieces.length > 0) {
    return pieces.join(' ');
  }
