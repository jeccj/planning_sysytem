...
export function parseDateTimeWithTimezone(input: string, fieldName: string): Date {
    if (!input || typeof input !== 'string') {
        throw new BadRequestException(`${fieldName} is required`);
    }
    if (!hasExplicitTimezone(input)) {
        throw new BadRequestException(`${fieldName} must include timezone offset (Z or +/-HH:mm)`);
    }
