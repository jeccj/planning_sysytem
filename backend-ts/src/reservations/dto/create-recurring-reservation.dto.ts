...
export class RecurrenceRuleDto {
  ...
  @IsDateString()
  @Matches(/(Z|[+-]\d{2}:\d{2})$/, {
    message: 'until must include timezone offset (Z or +/-HH:mm)',
  })
  @IsOptional()
  until?: string;
...
export class CreateRecurringReservationDto {
  ...
  @IsDateString()
  @Matches(/(Z|[+-]\d{2}:\d{2})$/, {
    message: 'start_time must include timezone offset (Z or +/-HH:mm)',
  })
  start_time: string;

  @IsDateString()
  @Matches(/(Z|[+-]\d{2}:\d{2})$/, {
    message: 'end_time must include timezone offset (Z or +/-HH:mm)',
  })
  end_time: string;
