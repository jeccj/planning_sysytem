...
import { buildSlotWindows, isBlockingStatus, isUniqueSlotError } from './utils/slot-utils';
...
export class ReservationsService {
    ...
    async findOne(id: number): Promise<Reservation | null> {
        return this.reservationRepository.findOne({ where: { id }, relations: ['venue'] });
    }
    ...
    async createBatch(createBatchDto: CreateBatchReservationDto, userId: number) {
        ...
        if (items.length > MAX_BATCH_ITEMS) {
            throw new BadRequestException(`Batch size cannot exceed ${MAX_BATCH_ITEMS}`);
        }
        ...
        if (allOrNothing) {
            ...
            try {
                ...
                for (const item of items) {
                    const row = await this.createPendingReservation(item, userId, undefined, queryRunner.manager);
                    created.push(row);
                }
    ...
    async createRecurring(createRecurringDto: CreateRecurringReservationDto, userId: number) {
        const occurrences = this.buildRecurringDrafts(createRecurringDto);

        const createdIds: number[] = [];
        const failedItems: Array<{ start_time: string; end_time: string; reason: string }> = [];
    ...
    async updateStatus(id: number, updateReservationDto: UpdateReservationDto, actor: ReservationActor): Promise<Reservation> {
        ...
        if (actor.role === UserRole.STUDENT_TEACHER) {
            ...
            if (
                ...
                !(reservation.status === ReservationStatus.APPROVED && reservation.startTime > now)
            ) {
                throw new BadRequestException('Current reservation status cannot be canceled');
            }

            return this.persistStatusWithSlots(id, targetStatus, updateReservationDto.rejection_reason || '');
        }
        ...
        if (targetStatus === ReservationStatus.REJECTED && !updateReservationDto.rejection_reason?.trim()) {
    ...
    private async createPendingReservation(
        ...
        const reservationRepo = manager?.getRepository(Reservation) || this.reservationRepository;
        ...
        if (draft.attendees_count > venue.capacity) {
            throw new BadRequestException(`Attendees exceed venue capacity (${venue.capacity})`);
        }

        const overlap = await this.findOverlappingReservationInRepository(reservationRepo, draft.venue_id, start, end);
        if (overlap) {
            throw new BadRequestException(overlap.status === ReservationStatus.MAINTENANCE
    ...
    private async upsertBlockingSlots(manager: EntityManager, reservation: Reservation): Promise<void> {
        const slotRepo = manager.getRepository(ReservationSlot);
        await slotRepo.delete({ reservationId: reservation.id });

        const windows = buildSlotWindows(reservation.startTime, reservation.endTime);
        if (windows.length === 0) {
            return;
        }

        const rows = windows.map((window) => slotRepo.create({
    ...
    private assertTimeRangeValid(start: Date, end: Date) {
        ...
        if (durationMinutes < MIN_DURATION_MINUTES) {
            throw new BadRequestException(`Reservation duration must be at least ${MIN_DURATION_MINUTES} minutes`);
        }
    ...
    private buildRecurringDrafts(createRecurringDto: CreateRecurringReservationDto): ReservationDraft[] {
        const baseStart = parseDateTimeWithTimezone(createRecurringDto.start_time, 'start_time');
        const baseEnd = parseDateTimeWithTimezone(createRecurringDto.end_time, 'end_time');
        this.assertTimeRangeValid(baseStart, baseEnd);

        const recurrence = createRecurringDto.recurrence;
        const interval = recurrence.interval || 1;
        const until = recurrence.until ? parseDateTimeWithTimezone(recurrence.until, 'recurrence.until') : null;

        if (!recurrence.occurrences && !until) {
            throw new BadRequestException('Either recurrence.occurrences or recurrence.until is required');
        }

        const desiredOccurrences = Math.min(recurrence.occurrences || MAX_RECURRING_OCCURRENCES, MAX_RECURRING_OCCURRENCES);
        ...
        if (recurrence.frequency === RecurrenceFrequency.DAILY) {
            for (let i = 0; rows.length < desiredOccurrences; i++) {
                const start = new Date(baseStart.getTime() + i * interval * 24 * 60 * 60 * 1000);
        ...
        const weekdays = recurrence.week_days && recurrence.week_days.length > 0
            ? Array.from(new Set(recurrence.week_days)).sort((a, b) => a - b)
            : [baseStart.getUTCDay()];

        const baseDateUtc = Date.UTC(baseStart.getUTCFullYear(), baseStart.getUTCMonth(), baseStart.getUTCDate());
        const cursor = new Date(baseDateUtc);
        const maxUntil = until || new Date(baseStart.getTime() + 400 * 24 * 60 * 60 * 1000);
    ...
    private withDraftTime(source: CreateRecurringReservationDto, start: Date, end: Date): ReservationDraft {
        return {
