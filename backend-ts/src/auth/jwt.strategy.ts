...
const SESSION_IDLE_TIMEOUT_MS = Number(process.env.SESSION_IDLE_TIMEOUT_MS || 1000 * 60 * 60 * 24 * 14);
const SESSION_TOUCH_INTERVAL_MS = Number(process.env.SESSION_TOUCH_INTERVAL_MS || 1000 * 60 * 5);
...
export class JwtStrategy extends PassportStrategy(Strategy) {
    ...
    async validate(payload: any): Promise<User> {
        ...
        const maintenance = await this.configRepository.findOne({ where: { key: IMPORT_MAINTENANCE_ACTIVE_KEY } });
        const isMaintenanceActive = String(maintenance?.value || '').trim().toLowerCase() === 'true';
        ...
        const isIdleExpired = lastActiveAtMs > 0 && Number.isFinite(lastActiveAtMs) && (now - lastActiveAtMs > SESSION_IDLE_TIMEOUT_MS);
        if (isIdleExpired) {
        ...
        const shouldTouchActivity = !lastActiveAtMs || !Number.isFinite(lastActiveAtMs) || (now - lastActiveAtMs > SESSION_TOUCH_INTERVAL_MS);
