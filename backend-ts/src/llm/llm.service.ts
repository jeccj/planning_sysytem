...
export class LlmService {
  ...
  private async initClient() {
    // 缓存未过期则跳过重新初始化
    if (
      Date.now() - this.configCachedAt < this.CONFIG_TTL_MS &&
    ...
    this.provider =
      (await this.configService.findByKey('llm_provider')) || 'gemini';
    const apiKey =
      (await this.configService.findByKey('llm_api_key')) ||
      process.env.GEMINI_API_KEY;
    const baseUrl = await this.configService.findByKey('llm_base_url');
    this.modelId =
      (await this.configService.findByKey('llm_model')) ||
      (this.provider === 'gemini' ? 'gemini-2.0-flash-lite' : 'deepseek-chat');
    this.systemPrompt = (
      (await this.configService.findByKey('llm_system_prompt')) ||
      DEFAULT_LLM_SYSTEM_PROMPT
    ).trim();
    this.jsonGuardPrompt = (
      (await this.configService.findByKey('llm_json_guard_prompt')) ||
      DEFAULT_LLM_JSON_GUARD_PROMPT
    ).trim();
    this.parseIntentRules =
      (await this.configService.findByKey('llm_parse_intent_rules')) ||
      DEFAULT_LLM_PARSE_INTENT_RULES;
    this.auditRules =
      (await this.configService.findByKey('llm_audit_rules')) ||
      DEFAULT_LLM_AUDIT_RULES;
    this.expandProposalRules =
      (await this.configService.findByKey('llm_expand_proposal_rules')) ||
      DEFAULT_LLM_EXPAND_PROPOSAL_RULES;
  ...
  private async callLlm(prompt: string): Promise<string> {
    ...
    if (
      (this.provider === 'deepseek' || this.provider === 'custom') &&
      this.openaiClient
    ) {
      ...
      try {
        response = await fetch(
          `${this.openaiClient.baseUrl}/v1/chat/completions`,
          {
            method: 'POST',
            headers: {
      ...
      throw new Error(
        'DeepSeek/Custom API invalid response: ' + JSON.stringify(data),
      );
    }
  ...
  private extractBuildingKeyword(queryText: string): string | undefined {
    ...
    const zhMatch = text.match(
      /([A-Za-z0-9一二三四五六七八九十零〇两_-]{1,20}(?:楼|栋|馆|中心))/,
    );
    if (zhMatch?.[1]) {
  ...
  async parseIntent(text: string): Promise<IntentResult> {
    ...
    try {
        ...
        if (
          !keywords.some(
            (item) =>
              ...
              String(parsed.building).toLowerCase(),
          )
        ) {
      ...
      if (
        ...
        lowerText.includes('上课') ||
        lowerText.includes('classroom')
      ) {
        detectedType = 'Classroom';
      } else if (
        lowerText.includes('实验') ||
        lowerText.includes('lab') ||
        lowerText.includes('实验室')
      ) {
        detectedType = 'Lab';
      } else if (
        lowerText.includes('礼堂') ||
        lowerText.includes('演讲') ||
        lowerText.includes('hall') ||
        lowerText.includes('讲座')
      ) {
        detectedType = 'Hall';
      }
      ...
      const actMatch = text.match(
        /(?:举办|进行|开展|for|holding)\s*([^\s,，。]+)(?:活动|会议|讲座|比赛)?/,
      );
      if (actMatch) activity_name = actMatch[1];
      else if (text.includes('会议') || text.includes('开会'))
        activity_name = '会议';
      ...
      const orgMatch = text.match(
        /(?:由)?\s*([^\s,，。]+?)(?:举办|组织|organize)/,
      );
      if (
        orgMatch &&
        !['明天', '后天', '今天', '上午', '下午'].includes(orgMatch[1])
      ) {
  ...
  async auditProposal(text: string): Promise<AuditResult> {
    ...
    try {
      ...
      if (lowerText.includes('fire') || lowerText.includes('alcohol')) {
        return {
          score: 85,
          reason: 'High risk keywords detected (fire/alcohol).',
        };
      }
  ...
  async expandProposal(
    input: ProposalExpandInput,
  ): Promise<{ expanded_text: string }> {
    ...
    try {
      ...
      console.log(
        `[LLM Service] expandProposal fallback (${this.provider}): ${error}`,
      );
    }
  ...
  async explainVenueSearch(
    input: VenueSearchExplainInput,
  ): Promise<VenueSearchInsight> {
    await this.initClient();

    const assumptions = Array.isArray(input.defaultAssumptions)
      ? input.defaultAssumptions
      : [];
    const fallbackCriteria = this.buildSearchCriteria(
      input.intent || {},
      assumptions,
    );
    ...
    try {
      ...
      console.log(
        `[LLM Service] explainVenueSearch fallback (${this.provider}): ${error}`,
      );
      return {
