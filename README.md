...
- 修复学生端预约弹窗在竖屏下的“横向挤压/遮挡/可左右拖动”问题
- 预约模式在手机端改为独立纵向按钮（单次/批量/循环），避免连体按钮边框遮挡
- 预约时间在手机端改为单列输入（日期、开始、结束），避免文案与占位符被截断
- 预约弹窗内表单文本统一字号（移动端），提升视觉一致性
- 统一限制弹层横向滚动（overlay/dialog body `overflow-x` 收口），增强 iOS Safari 交互稳定性
- 修复楼栋二级场馆弹窗在窄屏下操作按钮越界与裁切问题
- 智能搜索 Demo 动画增加竖屏自适应半径与布局修正，保证与横屏一致的流程观感
