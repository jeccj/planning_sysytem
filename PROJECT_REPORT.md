...
3. **预约场馆**：查看"最佳结果"与"更多匹配"，点击"预约"按钮填写信息（系统会校验预约时段是否落在场馆开放时间内）
...
3. **场馆管理**：维护场馆信息、上传照片、设置维护时段；编辑"开放时间"后将影响搜索与预约校验
4. **用户管理**：维护角色与管理范围（仅系统管理员）
5. **系统设置**：配置 LLM、编辑提示词、CSV 导入（仅系统管理员）

> **开放时间格式示例**：`08:00-22:00`、`周一至周五 08:00-22:00`、`周六至周日 09:00-18:00`。支持使用 `;` 分段配置多条规则。
...
8. 不在场馆开放时间内的候选场馆直接剔除（默认开放时间 `08:00-22:00`）
